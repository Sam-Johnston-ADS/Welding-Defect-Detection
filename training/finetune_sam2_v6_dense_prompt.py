"""
SAM 2 + LoRA Fine-tuning — Cascade Stage 3.6 (SegFormer v6 Box Prompting)
Cascade: SegFormer v6 (Strip Pooling + ASPP) Box with 5% Padding -> SAM 2 Refinement

Run from terminal:
  python finetune_sam2_v6_box_prompt.py --epochs 50 --batch_size 4 --lr 1e-4
"""
...
# ─────────────────────────────────────────────
# 2. DATASET: Box Prompt Generation
# ─────────────────────────────────────────────
class WeldSAM2CascadeDataset(Dataset):
    ...
    @torch.no_grad()
    def _get_segformer_prompts(self, image_np):
        """Generates Box Prompt with 5% Padding."""
        h, w, _ = image_np.shape
        
        # ImageNet normalization matching v6 training
        ...
        if len(nonzero) == 0:
            # Fallback for negative frames (let SAM2 figure it out using the whole image space)
            return [0, 0, w, h]
        ...
        # -------------------------------------------------------------
        # PROMPT: Bounding Box WITH 5% PADDING
        # -------------------------------------------------------------
        ...
        x2 = min(w, int(max(x_min + 1, x_max) + pad_x))
        y2 = min(h, int(max(y_min + 1, y_max) + pad_y))
    ...
    def __getitem__(self, idx):
        ...
        # Retrieve box prompt
        box = self._get_segformer_prompts(image_np)
...
# ─────────────────────────────────────────────
def main():
    parser = argparse.ArgumentParser(description="SAM 2 LoRA Fine-Tuning Guided by SegFormer v6 Box Prompts")
    ...
    SAVE_PATH         = os.path.join(MODEL_DIR, "sam2_lora_best_v6_box.pth")
    ...
    # 1. Load SegFormer v6
    print("\nBuilding SegFormer-B2 v6 Prompt Generator...")
    ...
    # 5. Training Loop
    print(f"\nStarting SAM 2 LoRA fine-tuning (Guided by v6 Box Prompts)...\n")
    best_iou = 0.0
    history  = []
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch in pbar:
            ...
            # Forward pass: removed mask_inputs
    ...
    with open(os.path.join(MODEL_DIR, "sam2_history_v6_box.json"), "w") as f:
        json.dump(history, f, indent=2)

    print(f"\n{'='*50}")
    print(f"SAM 2 Local Fine-tuning (v6 Box Prompt Cascade) Complete")
    print(f"{'='*50}")
